...
mod obsolete {
    ...
    pub enum OrchestratorInput {
        ...
        SetOrchestrator(Arc<Mutex<Orchestrator>>),
    }
                ...
                loop {
                    ...
                    if let Ok(mut o) = orchestrator.lock() {
                        ...
                        match recv {
                            Ok(input) => match input {
                                ...
                                OrchestratorInput::ProjectOpen(path) => {
                                    match Self::handle_input_load(&path) {
                                        ...
                                        Err(err) => {
                                            let _ = sender
                                                .send(OrchestratorEvent::LoadError(path, err));
                                        }
                                    }
                                    {}
                                }
                                OrchestratorInput::ProjectSave(path) => {
                                    match Self::handle_input_save(&o, &path) {
                                        ...
                                        Err(err) => {
                                            let _ = sender
                                                .send(OrchestratorEvent::SaveError(path, err));
                                        }
                                    }
                                }
                                ...
                                OrchestratorInput::TrackDeleteSelected => {
                                    if let Ok(track_selection_set) = track_selection_set.lock() {
                                        let uids =
                                            Vec::from_iter(track_selection_set.iter().copied());
                                        uids.iter().for_each(|uid| {
                                            let _ = o.delete_track(*uid);
                                        });
                                    }
                                }
                                ...
                                OrchestratorInput::TrackAddEntity(track_uid, key) => {
                                    ...
                                    if let Some(entity) = factory.new_entity(&key, uid) {
                                        let _ = o.add_entity(&track_uid, entity, Some(uid));
                                        let _ =
                                            o.connect_midi_receiver(uid, MidiChannel::default());
                                    }
                                }
                            ...
                            Err(err) => {
                                eprintln!(
                                    "unexpected failure of OrchestratorInput channel: {:?}",
                                    err
                                );
                                break;
                            }
